      ...
      watch:{
        message:{
          handler:function(val,oldval){
            ...
            if(type != undefined){
              localStorage.setItem("CARRIERSFETMESSAGE",JSON.stringify(_this.message));
            }
          },
          deep:true
        }
      ...
      mounted:function () {
        ...
        if(type != undefined){
           ...
           if(CARRIERSFETMESSAGE != undefined){
             _this.message = JSON.parse(CARRIERSFETMESSAGE);
           }
        }else{
          $.ajax({
            type: "POST",
            ...
            success: function(getCarrAndCompanyInfo){
              if(getCarrAndCompanyInfo.success == "1"){
               _this.message = {
                 ...
                 second:{
                   driverLicense:{
                     bendi: getCarrAndCompanyInfo.ftpUrl + getCarrAndCompanyInfo.driverLicense,
                     http: getCarrAndCompanyInfo.driverLicense,
                   },
                   drivingLicence:{
       ...
       methods:{
         ...
         goNext:function () {
           ...
           if(_this.nowStep < 2){
             ...
             var json = {
               userName : _this.message.first.name,
               ...
               type:1,
               driverLicense : _this.message.second.driverLicense.http ,
